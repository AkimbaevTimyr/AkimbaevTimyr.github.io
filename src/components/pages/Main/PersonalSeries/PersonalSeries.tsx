        ...
        <div className='ml-5'>
            ...
            <div className='mt-5 flex flex-wrap'>
                {personalSeries.slice(0, 11).map((el: IMovie) => (
                    <Link to={`tv/${el.id}`}><div key={el.id} className='ml-5 cursor-pointer w-28 '><img className='rounded-xl' src={`https://image.tmdb.org/t/p/w220_and_h330_face/${el.poster_path}`} /></div></Link>
                ))}
            </div>
        </div>
    )
